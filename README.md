...
**The key feature here is that this dictionary is dynamically adjusted.** For example, #job is overall the most frequently used hash tag, and it *is* related to the 2016 election. However, since it's so widely used in other contexts that are irrelevant to the 2016 election, thus this #job is usually excluded from our dictionary. By doing so, we would *not* loss any tweets that are both relevant to job market and the 2016 election, since such tweets would most likely call other tags that are more exclusively related to our focus. But without excluding #job, we would fetch up so many tweets that are irrelevant to our topic under study. Please note that our approach is quite different from [Ref2](https://github.com/Nimburg/Ultra_Phase1v5_Phase2v2_Phase3v1/blob/master/Results_Demo/A%20system%20for%20real-time%20Twitter%20sentiment%20analysis%20of%202012%20U.S.%20presidential%20election%20cycle.pdf) where the set of rules of filtering data is set, while my approach would adjust to new hash tags and new topics that arise during a campaign.
...
**first, their messages are very succinct**. This means twitter users have to express whatever they want to say through those commonly used keywords and hash tags. Thus, information is highly concentrated, to the point where, if one get the few keywords right, one could get the entire message right. 
**second, they use hash tag**. In addition to the first point, the use of hash tag also normalized twitter users' language. In other words, there is little need for us to normalize twitter users' languages because: 1st, their key points are concentrated in the few hash tags they called; 2nd, hash tags are already normalized.
**third, tweet messages are context-heavy**. People write tweets to communicate with their friends, with whom they share a lot of time together. Thus, twitter user usually don't need to elaborate on what they are talking about; instead, they focus on what they feel about the commonly known topics. Words like "tax paper", "emails", "Benghazi" are much more *meaningful* that they usually are. However, traditional NLP methods won't be able to catch it. 
...
Naturally, one could introduce a nonlinear method of approximating the conditional probability among hash tags. Take "#vote4trump #vote" and "#vote4trump #vote #imwithher" for example. The latter combination represents an abnormal case, since people should not express support for both parties at the same time, given the bipartisan nature of election. It is more likely that this user used some more complex expression method like irony or exaggeration, resulting in calling #vote4trump and #imwithher at the same time. Thus, a good nonlinear model should be able to place less weight on this kind of cases. Nonlinear model like this would be an improve over my current linear model (*and I intend to work on such nonlinear method in the coming weeks*). However, I doubt how much improvement one could achieve. Since we are working on a *classification problem*, all we need is to create a separation among the data; as to the absolute value of such separation, it really doesn't matter.
