...
The Long Short Term Memory (LSTM) method has been in itself a very powerful method for NLP related applications. There has been [many](http://www.wildml.com/2015/10/recurrent-neural-network-tutorial-part-4-implementing-a-grulstm-rnn-with-python-and-theano/) [posts](http://colah.github.io/posts/2015-08-Understanding-LSTMs/) elaborating how it works as well as [why](http://karpathy.github.io/2015/05/21/rnn-effectiveness/) it is so powerful. Here, the motivation for me to train LSTM on a "semi-automatically" generated corpus is a little different. Unlike an ordinary supervised method, where one already got a "correct" corpus for training and expect the LSTM to catch the structure and keywords of each sentences, here I use the LSTM as a method of "expanding the reference tokens". For example, the iteration operation introduced in previous section would "expand the reference token" from ~200 hand-marked hash tags to ~1000 hash tags. Similarly, training a LSTM on a semi-automatically generated corpus, if allowing the training process to run deep, the LSTM may allocate all its weights onto those ~1000 hash tags, since the training corpus is generated using those hash tags. However, if one would stop the training process early, then the weight will be spread out onto other keywords, high-frequency words that is closely associated with hash tags, thus expanding the set of reerence tokens. 

The picture between is the top 150 most frequently used words from the corpus, where there are ~ 60000 unique words. One could see ~20% of them are hash tags. At the same time, among those high frequence words, there are words like "no, not, never, dont, cant, etc" expressing negative sentiments (correspondingly there are word expressing positive sentiments). If we follow our prevous assumptions that words showing positive sentiment are statistically used more in messages expressing positive sentiments, and if we could allow the LSTM to keep some weights on those words, then we would be able to spot unusual cases where many negative words are used with several positive hash tags (for example, expression of satire). In short, I am trying to use the LSTM in a slightly unsupervised way; or put it in another way, I am mainly exploiting its capability of finding correlations between sequences of data. 

![alt tag](https://github.com/Nimburg/Ultra_Phase1v5_Phase2v2_Phase3v1/blob/master/Results_Demo/Dictionary_Freq.png)

A few notes: my corpus for LSTM has 86979/4578/22888 sentences for training/validation/testing. For a deep training (~2.5% error for validationa and testing), the number of mismatches between LSTM prediction and prediction solely from tags is ~4000 sentences. For a shallow training with early stop (~3.5% error for validationa and testing), the number of misatches is 
